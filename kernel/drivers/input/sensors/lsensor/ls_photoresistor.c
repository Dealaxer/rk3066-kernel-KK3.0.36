...
static int sensor_active(struct i2c_client *client, int enable, int rate)
{
    ...
    //printk("light_photoresistor: sensor active is %s \n",( (enable)?"on":"off" ) );
...
static void lsp_adc_callback(struct adc_client *client, void *callback_param, int result)
{
	...
	//printk("light_photoresistor: adc callback value is %d \n",glspdata.adcvalue);
}
...
static int sensor_report_value(struct i2c_client *client)
{
	...
	if(glspdata.adcvalue==0||glspdata.adcvalue==1){//
		printk("light_photoresistor: adc value is (%d)  invalid \n",glspdata.adcvalue);
	    	return ret ;
	}
	  
	if(glspdata.adcvalue != glspdata.oldadcvalue)
	{
		DBG("light_photoresistor: input report value is %d \n",glspdata.adcvalue);
...
static int sensor_init(struct i2c_client *client)
{
    ...
    if( (sensor->pdata->address >= 0) && (sensor->pdata->address <=3) ){
	    ...
	    if( !glspdata.adc_client ) {
			printk("%s : light_photoresistor adc_register faile !\n",__func__);
			error = -EINVAL;
			goto fail1;
	     }
    }
